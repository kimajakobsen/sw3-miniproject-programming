...
Our program manages manufactures, products and cart.
...
The commands allows the user to filter the list, e.g. by applying a price range.

It was not part of the assignment to an upper bound for storage capacity, we did however add such a filter, to make the filter by capacity resemble the filter by price.
...
According to the assignment, we should make sure only one cart exist, therefore the Cart class use the singleton pattern. The cart menu prints the content of the cart according to the specifications of our assignment.
...
It is not possible to remove a manufacturer which is associated to a product, since this would result in a product without a manufacturer.
